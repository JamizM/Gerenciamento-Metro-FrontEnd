...
export default async function findExtinguisherByLocation(localizacao: Number) {
    const base64Credentials = btoa("Admin:Admin");
    const url = `http://192.168.238.143:8080/api/Extinguishers?localization=${localizacao}`; //url
    try {
